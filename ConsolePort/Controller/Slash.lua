...
SLASH_FUNCTIONS = {
	...
	config = function(path, value)
		...
		if not config then
			return CPAPI.Log("No active config found.")
		end
	...
	status = function(deviceID)
		...
		if next(activeDevices) then
			...
			for _, device in ipairs(activeDevices) do
				...
				CPAPI.Log(
					"   Vendor: |cFF00FFFF%s|r, Product: |cFF00FFFF%s|r, Config: %s",
					vendorID,
					productID,
	...
	resetall = function()
		CPAPI.Popup(
			"ConsolePort_Uninstall_Settings",
			{
